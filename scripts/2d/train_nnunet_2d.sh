...
# Second stages use scripts/2d/export.py then scripts/2d/train_coarse_to_fine.sh, train_multiview.sh, and/or train_uncertainty.sh.
# Skip preprocess if cache is valid:
#   bash scripts/2d/train_nnunet_2d.sh --skip-preprocess
# or: SKIP_NNUNET_PREPROCESS=1 bash scripts/2d/train_nnunet_2d.sh
set -euo pipefail
