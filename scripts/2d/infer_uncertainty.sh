#!/usr/bin/env bash
# Full-volume inference: nnU-Net + UncertaintyUNet2d refinement (see scripts/2d/infer_uncertainty.py).
#
# Example:
#   bash scripts/2d/infer_uncertainty.sh -i nnUNet_raw/Dataset001_LiverTumor/imagesTr \
...
exec "${PYTHON}" "${REPO_ROOT}/scripts/2d/infer_uncertainty.py" "$@"
