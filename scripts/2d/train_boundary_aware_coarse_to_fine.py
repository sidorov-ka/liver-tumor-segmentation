...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Directory with train/ and val/ .npz from scripts/2d/export.py (same as coarse_to_fine).",
    )
    p.add_argument(
        "--out-dir",
        type=str,
...
def main() -> None:
    ...
    if len(train_ds) == 0:
        raise ValueError("train/ has no .npz files — run scripts/2d/export.py first.")
    if len(val_ds) == 0:
        raise ValueError(
            "val/ has no .npz files. Run scripts/2d/export.py with nnU-Net splits_final.json "
