...
produced by ``scripts/2d/train_multiview.py`` (``results_multiview/.../multiview/run_*``).
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        default=0.75,
        help="nnU-Net sliding-window step (match scripts/2d/infer_coarse_to_fine.py / export).",
    )
    ...
    p.add_argument(
        ...
        help="nnU-Net trainer checkpoint inside model-dir (match scripts/2d/infer_coarse_to_fine.py).",
    )
    p.add_argument(
        "--save-probabilities",
        action="store_true",
