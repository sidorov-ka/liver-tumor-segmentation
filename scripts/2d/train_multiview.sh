...
#   EXPORT_DIR=/path/to/slices bash scripts/2d/train_multiview.sh
# Interpreter: PYTHON=python3 (default) or PYTHON=python
# Extra args are passed through: bash scripts/2d/train_multiview.sh --epochs 50
set -euo pipefail
...
exec "${PYTHON}" "${REPO_ROOT}/scripts/2d/train_multiview.py" --export-dir "${EXPORT_DIR}" "$@"
