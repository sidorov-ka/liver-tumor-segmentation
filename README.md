...
Мультиклассовая сегментация печени и опухоли на КТ с [nnU-Net v2](https://github.com/MIC-DKFZ/nnUNet) на датасете в формате nnU-Net (`Dataset001_LiverTumor`). **coarse_to_fine** — отдельный 2D U-Net по грубой маске nnU-Net. **multiview** — **независимая** надстройка: своя сеть `MultiviewUNet2d` (три фиксированных HU-окна + канал вероятности опухоли), в духе multi-view fusion для узлов на КТ (см. *Deep Multi-View Fusion Network for Lung Nodule Segmentation* и аналоги); чекпоинт обучается отдельно, не из весов coarse_to_fine. Код: `src/2d/multiview`, обучение `scripts/2d/train_multiview.py`, инференс `scripts/2d/infer_multiview.py`. **uncertainty** — ещё одна независимая вторая стадия: `UncertaintyUNet2d` (три HU-окна + вероятность + энтропия Bernoulli), `src/2d/uncertainty`, `scripts/2d/train_uncertainty.py`, `scripts/2d/infer_uncertainty.py`. **boundary_aware_coarse_to_fine** — отдельная вторая стадия: компактный 2D U-Net `BoundaryAwareTinyUNet2d` (по умолчанию **пять** входов: три HU-окна + вероятность опухоли stage-1 + нормированная энтропия; старые раны могли быть с тремя каналами — см. `meta.json`); на инференсе уточнение применяется **только в морфологическом кольце** вокруг грубой маски опухоли, с опционально адаптивной шириной кольца и порогом (`src/2d/boundary_aware_coarse_to_fine`, `scripts/2d/train_boundary_aware_coarse_to_fine.py`, `scripts/2d/infer_boundary_aware_coarse_to_fine.py`).
...
Скрипты сгруппированы: **`scripts/2d/`** (экспорт слайцев, вторые стадии 2D, полный объёмный инференс coarse_to_fine / multiview / uncertainty / boundary-aware), **`scripts/3d/`** (3D nnU-Net, multi-window, кэш вероятностей, слияние softmax), **`scripts/visualization/`** (matplotlib / PNG; артефакты по-прежнему удобно складывать в `visualizations/`). Точки входа stage-1 nnU-Net: **`scripts/2d/train_nnunet_2d.sh`** (2d) и **`scripts/3d/train_nnunet_3d.sh`** (3d_fullres). Метрики по NIfTI: **`scripts/evaluate_segmentations.py`**.

## Пайплайн (кратко)

1. **nnU-Net** — планирование, препроцесс, обучение fold 0 (`scripts/2d/train_nnunet_2d.sh`).
2. **Export** — слайсы с предсказанием stage-1 и GT для вторых стадий (`scripts/2d/export.py`; общий экспорт для coarse_to_fine, multiview, uncertainty и boundary_aware_coarse_to_fine).
3. **coarse_to_fine** — вторая стадия (`scripts/2d/train_coarse_to_fine.sh` или `train_coarse_to_fine.py` → `results_coarse_to_fine/...`).
4. **multiview** — вторая стадия (`scripts/2d/train_multiview.sh` или `train_multiview.py` → `results_multiview/.../multiview/run_*`).
5. **uncertainty** — вторая стадия (`scripts/2d/train_uncertainty.sh` или `train_uncertainty.py` → `results_uncertainty/.../uncertainty/run_*`).
6. **boundary_aware_coarse_to_fine** — вторая стадия (`scripts/2d/train_boundary_aware_coarse_to_fine.sh` → `results_boundary_aware_coarse_to_fine/.../boundary_aware_coarse_to_fine/run_*`).
7. **Инференс coarse_to_fine** — полный объём: только nnU-Net или nnU-Net + coarse_to_fine (`scripts/2d/infer_coarse_to_fine.py`).
8. **Инференс multiview** (опционально) — ROI + multi-window (`scripts/2d/infer_multiview.py --multiview-dir ...`; e.g. `inference_comparison/multiview/`).
9. **Инференс uncertainty** (опционально) — `scripts/2d/infer_uncertainty.py --uncertainty-dir ...` (по умолчанию `-o inference_comparison/uncertainty`).
10. **Инференс boundary_aware_coarse_to_fine** (опционально) — `scripts/2d/infer_boundary_aware_coarse_to_fine.py --boundary-aware-dir ...` (по умолчанию вывод под `inference_comparison/boundary_aware_coarse_to_fine/`). Не смешивать каталоги чекпоинтов разных стадий.
...
```bash
bash scripts/2d/train_nnunet_2d.sh
```
...
```bash
bash scripts/2d/train_nnunet_2d.sh --skip-preprocess
# или: SKIP_NNUNET_PREPROCESS=1 bash scripts/2d/train_nnunet_2d.sh
```
...
```bash
bash scripts/3d/train_nnunet_3d.sh
...
```bash
bash scripts/3d/revalidate_3d_boundary_shape_runs.sh
# только раны, в имени которых есть подстрока:
# RUN_FILTER=20260509 bash scripts/3d/revalidate_3d_boundary_shape_runs.sh
...
`scripts/3d/run_nnunet_with_local_3d_trainers.py` делает их видимыми для
...
```bash
python3 scripts/2d/export.py --output-dir refinement_export/fold0
```
...
# или вручную: python3 scripts/3d/cache_tumor_prob_for_multiwindow.py --model-dir <...__3d_fullres> --output-dir <cache> --fold 0 --split all
```

Дальше 3D multi-window: `bash scripts/3d/train_3d_multiwindow_refinement.sh --skip-preprocess` (по умолчанию ран `results_3d_multiwindow_runs/<дата_время>_multiwindow_3d_fullres/`, target spacing как у `train_nnunet_3d.sh` — **1.25 1.0 1.0**; см. `src/3d/multiwindow/README.md`, переопределение через `NNUNET_MW_TARGET_SPACING`).
...
```bash
bash scripts/2d/train_coarse_to_fine.sh
# эквивалентно: python3 scripts/2d/train_coarse_to_fine.py --export-dir refinement_export/fold0
# другой экспорт: EXPORT_DIR=/path/to/slices bash scripts/2d/train_coarse_to_fine.sh
```
...
```bash
bash scripts/2d/train_multiview.sh
# эквивалентно: python3 scripts/2d/train_multiview.py --export-dir refinement_export/fold0
# другой экспорт: EXPORT_DIR=/path/to/slices bash scripts/2d/train_multiview.sh
```

По умолчанию логи и чекпоинты: `results_multiview/<dataset>/fold_<n>/multiview/run_<timestamp>/`  
(зеркально coarse_to_fine: папка задачи `multiview` внутри fold). Дальше: `python scripts/2d/infer_multiview.py ... --multiview-dir <этот_ран>`.
...
```bash
bash scripts/2d/train_uncertainty.sh
# эквивалентно: python3 scripts/2d/train_uncertainty.py --export-dir refinement_export/fold0
# другой экспорт: EXPORT_DIR=/path/to/slices bash scripts/2d/train_uncertainty.sh
```

Дальше: `python3 scripts/2d/infer_uncertainty.py ... --uncertainty-dir <каталог_с_checkpoint_best.pth_и_meta.json>` (по умолчанию `-o inference_comparison/uncertainty`). Справка: `python3 scripts/2d/infer_uncertainty.py -h`.

### 4.2. Обучение boundary_aware_coarse_to_fine (BoundaryAwareTinyUNet2d)

Тот же `--export-dir`, что в разделе 2. Обучение на ROI **GT ∪ coarse** (как coarse_to_fine), resize в `--crop-size`. **Пять входов** (по умолчанию): три HU-окна (`boundary_aware_coarse_to_fine.config`, по смыслу согласованы с multiview/uncertainty) + `coarse_tumor_prob` + нормированная энтропия Bernoulli по этой вероятности. Опции: `--hu-windows` (шесть чисел W/L × 3), `--lambda-boundary` (вес BCE+Dice на **кольце границы** вокруг грубой маски), `--focal-gamma` (фокальный BCE), `--bce-weight`, `--boundary-dilate-iters` / `--boundary-erode-iters`, см. `python3 scripts/2d/train_boundary_aware_coarse_to_fine.py -h`.

```bash
bash scripts/2d/train_boundary_aware_coarse_to_fine.sh
# эквивалентно: python3 scripts/2d/train_boundary_aware_coarse_to_fine.py --export-dir refinement_export/fold0
```
...
- **Только nnU-Net (baseline):** тот же стек предиктора, что и во второй стадии, с шагом **0.75** по умолчанию:
  ...
  ```bash
  python scripts/2d/infer_coarse_to_fine.py --stage1-only -i <папка_с_КТ_как_в_nnUNet_raw> -o <куда_писать_сегментации>
  ```

- **Две стадии (nnU-Net + coarse_to_fine):**

  ```bash
  python scripts/2d/infer_coarse_to_fine.py -i <вход> -o <выход> --coarse-to-fine-dir <папка_рана_с_meta.json_и_checkpoint_best.pth>
  ```

Общие флаги: `--tile-step-size`, `--no-mirroring`, `--fold`, `--model-dir`, `--export-stage1-to` (опционально сохранить stage-1), **`--save-probabilities`** (полный softmax nnU-Net: `case_id.npz` + `case_id.pkl` + stage-1 `case_id.nii.gz`; для двух стадий по умолчанию каталог `<out_dir>/nnunet_stage1_softmax`, чтобы не перезаписать финальную маску). Для метрик на **валидации** того же fold: `--split val` (список кейсов из `nnUNet_preprocessed/.../splits_final.json`). После OOM или обрыва: тот же `-o` и **`--skip-existing`**, чтобы не пересчитывать готовые кейсы. Справка: `python scripts/2d/infer_coarse_to_fine.py -h`.

**Слияние двух softmax (одинаковая геометрия):** если для двух прогонов сохранены `*.npz` с ключом `probabilities`, можно смешать их и получить одну жёсткую сегментацию на кейс:

```bash
python scripts/3d/infer_fuse_softmax_blend.py \
  --model-dir nnUNet_results/Dataset001_LiverTumor/nnUNetTrainer_150__nnUNetPlans_3d_midres125__3d_fullres \
  --softmax-dir-a <путь/к/nnunet_stage1_softmax_ран_A> \
  --softmax-dir-b <путь/к/nnunet_stage1_softmax_ран_B> \
  --lambda 0.5 \
  -o fused_segmentations/
```
...
```bash
python scripts/2d/infer_multiview.py -i <imagesTr_или_аналог> -o <выход> --multiview-dir <results_multiview/.../multiview/run_<timestamp>>
```

Справка: `python scripts/2d/infer_multiview.py -h`.
...
```bash
python3 scripts/2d/infer_uncertainty.py -i <imagesTr_или_аналог> --uncertainty-dir <results_uncertainty/.../uncertainty/run_<timestamp>> -o inference_comparison/uncertainty
```

Справка: `python3 scripts/2d/infer_uncertainty.py -h`.
...
```bash
python3 scripts/2d/infer_boundary_aware_coarse_to_fine.py \
...
Справка: `python3 scripts/2d/infer_boundary_aware_coarse_to_fine.py -h`.
...
└── scripts/
    ├── 2d/                       # train_nnunet_2d.sh (stage-1 2d), export, вторые стадии, infer_*, …
    ├── 3d/                       # train_nnunet_3d.sh, multi-window, кэш prob, infer_fuse_softmax_blend, …
    ├── visualization/            # plot / PNG
    └── evaluate_segmentations.py
```

Устаревшая плоская схема `scripts/*.py` перенесена в `scripts/2d/`, `scripts/3d/`, `scripts/visualization/`; см. раздел «Скрипты» выше.
